...
{
	print_if(__gOSC_debug, "built-in default handler called\n");
	return 0;
}
...
float handleOSCMessage(string messagePath, list messageArgs)
{
	...
	if (__gMessageHandlerMap.contains(messagePath)) {
		...
		print_if(__gOSC_debug, "messageHandler is %z\n", messageHandler);
		if (messageHandler != 0) {
...
// oscEnableLogging() enables and disables the OSC server logging for all messages.

float oscEnableLogging(int enable)
{
	__gOSC_debug = (enable != 0);
	print_if(__gOSC_debug, "Logging is now enabled\n");
	return 0;
}
