...
frameCount = dataset("./28poles.lpc",0)
...
start += (dur + 1);
frameCount = dataset("./36poles.lpc",0)

lpcstuff(thresh = .09, randamp = .1, 0, 0,0,0)
set_thresh(buzthresh = 0.09, noisethresh = 0.1);
frame2=frameCount-1
dur=(frame2-frame1)/fps
LPCPLAY(start,dur,amp,transp = .00001,frame1,frame2,warp,cf,bw)
...
start += (dur + 1);
frameCount = dataset("./56poles.lpc",0)
