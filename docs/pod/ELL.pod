...
=item B<p4> (I<ringdowndur>) ring-down duration - how long to run
the filter after it has consumed all the input 
...
Adapted by John Gibson E<lt>johgibso@indiana.eduE<gt> from the
