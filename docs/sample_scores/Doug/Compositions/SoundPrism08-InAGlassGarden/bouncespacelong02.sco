...
offset = 0;
totallength = irand(10, 25);

while (offset < totallength && minoffset > 0.01) {
	sBounceFactor = irand(0.7, 0.98);

	ldur = irand(0.5, 1.8);

	sFudgeDiff  = pickrand(-400, -312, -254, -213, -472, -513) + 400;
	energy = trand(40, 70);	// how "hard" the glass is hit
	hardness = irand(minhardness, maxhardness);
		
	size = pickrand(63, 65, 71, 87, 94) - 20;
	start = offset
	while (start - offset < ldur) {
		nextstart = start + irand(minnextdiff, maxnextdiff);		// time for next glass call
	...
	start = offset + irand(0.01, 0.04);
	while (start - offset < ldur) {
		nextstart = start + irand(minnextdiff, maxnextdiff);		// time for next glass call
		endtime = glass(start, nextstart - start, energy+3, hardness, size);
		start = nextstart;
	}

	offset += irand(minoffset, maxoffset);
