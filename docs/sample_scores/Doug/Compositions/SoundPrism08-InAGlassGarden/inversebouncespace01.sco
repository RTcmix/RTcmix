					...
					float durfactor, float gainfactor)
{
	...
	localstart = start;
	while (spacing < 1 && localstart < lastStart) {
		amp = gain * env
...
float glass(float outskip, float totalDur, float energy, float material, float size)
{
	...
	freq = primeFrequencies[frqindex] / 1.6;
...
energy = trand(70, 80);	// how "hard" the glass is hit
hardness = irand(80, 89);
	
size = pickrand(57, 63, 65, 71, 87, 94);
start = 0
startAngle = 3.0;
endAngle = irand(-90, 90);

dur = irand(2, 6);

endtime = glass(start, dur, energy, hardness, size);
roomMoveArc(start, 0, endtime, 6, startAngle, endAngle);

roomRun(endtime+1, 0.15);
