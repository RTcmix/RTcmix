...
_glassEnergyDurTable = maketable("expbrk", "nonorm", 1000, 0.7, 1000, 10.0);			// factor for dur of hits
...
float glass(float outskip, float totalDur, float energy, float material, float size)
{
	...
	frqindex = 5 + trunc(size * len(primeFrequencies) / 100);
...
while (start < 60) {
	sBounceFactor = irand(2.1, 2.7);
	energy = irand(70, 90);	// how "hard" the glass is hit
	hardness = irand(65, 89);
	
	size = irand(1,11);
	startAngle = irand(-90, 90);
	endAngle = startAngle + irand(-90, 90);

	dur = irand(15, 38);

	endtime = glass(start, dur, energy, hardness, size);
	roomMoveArc(start, 0, endtime, 12, startAngle, endAngle);
	start += irand(1, 4);
}

roomRun(endtime+2, 0.2);
