...
pitchtab = { 5.00, 5.01, 5.02, 5.03, 5.05, 5.07, 5.075, 5.10, 5.11 }
...
for (st = 0; st < totdur; st += incr) {
   ...
   retuned_pitch = partch(pitch) // use Harry Partch's 11-limit tuning
   amp = ampdb(minampdb + (ampdiff * random()))
   WAVETABLE(st, notedur, amp * env, retuned_pitch, pan=random(), wavet)
}
