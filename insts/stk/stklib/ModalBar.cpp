// ModalBar.C -- hacked version (by BGG) for RTcmix from Perry/Gary's STK
// BGG -- be sure to see how the tick(...) method works for RTcmix in Modal.C
...
ModalBar :: ModalBar()
  : Modal()
{
// BGG -- ok, this is the biggest change for RTcmix.  We will generate
// the excitation function in the RTcmix instrument, and then pass in
// the samples at run-time via the tick(...) method (in Modal.C)
/*
...
// BGG -- since the wave (WvIn) excitation isn't being used, the
// stick hardness will have to be handled in RTcmix.  Filtered noise burst?
// leave in the masterGain, though -- setPreset uses it
...
/* BGG -- vibraphone vibrato needs to be done in RTcmix (AM)
