// BlowHole.C -- hacked version (by BGG) for RTcmix from Perry/Gary's STK
...
BlowHole :: BlowHole(MY_FLOAT lowestFrequency)
{
  ...
  filter = new OneZero;
// BGG -- envelope taken out for replacement by RTcmix makegen
...
// BGG -- use noteOn in RTcmix to set up freq + amp (pressure)
...
MY_FLOAT BlowHole :: tick(float ampPressure)
{
...
void BlowHole :: setReedStiffness(MY_FLOAT value) // 0.0-1.0
{
	reedTable->setSlope( -0.44 + (0.26 * value) );
}

void BlowHole :: setNoise(MY_FLOAT value) // 0.0-1.0
{
	noiseGain = ( value * 0.4);
}
